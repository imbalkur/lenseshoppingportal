...
<html>
	...
	<body><?php include("header.php"); ?><br /><br /><br />
    ...
    if(strlen($query) >= $min_length){   
        ...
        $raw_results = mysqli_query($conn,"SELECT * FROM product WHERE (ptitle LIKE '%".$query."%') OR (pkeywords LIKE '%".$query."%')") or die(mysql_error());
        if(mysqli_num_rows($raw_results) > 0){       
            while($row= mysqli_fetch_array($raw_results)){
                ?>
				<div class="col-md-3">
			<form method="post" action="search.php?action=add&id=<?php echo $row["pid"]; ?>">
